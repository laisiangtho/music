...
class AudioBucketType {
  ...
  Future<void> artistInit() async{
    for (var index = 0; index < artist.length; index++) {
      final e = artist[index];
      if (index > 1){
        if (e.track == 0){
          List<AudioTrackType> trk = track.where(
            (e) => e.artists.contains(index)
          ).toList();
          Map<String,int> sum = trk.fold<Map<String,int>>({'plays':0,'duration':0, 'track':0}, (i, e) => {
  ...
  Iterable<int> artistPopularByLang(int id) => artist.where((e) => e.lang.length > 0 && e.lang.first == id).take(17).map((e) => e.id);
